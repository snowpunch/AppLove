...
//                self.button.loadImageWithUrl(Account.shared.mainUrl + newValue, defaultImageName: "home_default_login");
    ...
    func addDeleteButton(){
        self.createButton("", backgroundImage: WHC_MenuView.createImage(true , size: CGSizeMake(kWHCDeleteButtonRadius * 4.0, kWHCDeleteButtonRadius * 4.0)), sel: #selector(WHC_MenuItem.clickDeleteButton(_:)));
    }
    
    /// 添加插入按钮
    func addInsertButton(){
        self.createButton("", backgroundImage: WHC_MenuView.createImage(false , size: CGSizeMake(kWHCDeleteButtonRadius * 4.0, kWHCDeleteButtonRadius * 4.0)), sel: #selector(WHC_MenuItem.clickInsertButton(_:)));
    }
